...
\includepdf[pages=-]{mp_titlelist}
\includepdf[pages=-]{zadanibelike}

\newpage

\section*{Anotace}
V této maturitní práci se snažím o vytvoření programu který je schopný přednést příběh a předložit hráči možnosti na které příběh bude reagovat. Tímto se docílí interaktivita příběhu. Zároveň vše je děláno systémem scriptů, tudíž je možno hru, při dodržení pravidel pro psaní daných scriptů, měnit pomocí uživatelsky napsaných programů. Projekt také obsahuje metodu se kterou se dá zjednodušeně příběh napsat a použít jednotlivé scripty pro interaktivitu. Poslední část práce je uložení hráče a jeho odpovídajících scriptů na raspberry pi pico a čtení předem zmíněného obsahu ze zařízení. Celá práce je dělaná v programovacím jazyku Micropython.
...
Díky způsobu práce s daty na picu (nepracuje s nimi přímo uživatel, vše je ovládáno pomocí programu) tak může být na picu mnohem méně flexibilní struktura souborů. V rootu musí existovat main.py, který se sám spustí, když se pico zapojí. Zároveň musí být v rootu samotný soubor hráče a jakékoli knihovny, které main.py potřebuje. Následně všechny scripty, které jsou specifické vůči hráči, musí být v adresáři player\_scripts.
...
Hlavní program nastaví všechny iniciální hodnoty, provede inicializační sekvenci, najde místo příběhu a najde všechny eventy, které se budou muset stát. Všechny eventy mají své odpovídající scripty a hlavní program samozřejmě provede jejich import. Zároveň vytvoří list importovaných funkcí aby s nimi metody a funkce mohli pracovat.
...
Handler jako funkce má dva svoje parametry. První parametr musí být název scriptu bez koncovky .py (takto jsou uloženy v naimportovaných scriptech). Druhý parametr je pole argumentů, které chceme do funkce poslat. Handler používá rozbalovací znaménko * aby první dimenze pole byla předána funkci jako jednotlivé parametry. Zároveň se v této první dimenzi hledá \& znak, který jsem zvolil jako identifikátor že je třeba vzít hodnoty z paměti. Pokud nějakou najde hodnotu v paměti, nahradí \& znak odpovídající hodnotou.
...
Handler zjišťuje jestli funkce vrátila hodnotu, pokud ano tak ji ukládá do paměti. Je zde ale vyjímka, a to u mnou nazvaných logických scriptů. Logické scripty, pokud je jejich návratová hodnota typu slovník, mají schopnost přidávat eventy a tudíž jsou logické scripty schopné vytvářet smyčky. Handler kontroluje jestli jsou funkce importovány, aby nevolal neexistující funkce, a také kontroluje pro stringy s koncovkou .json. Pokud je takový string nalezen, tak Handler z odpovídajícího souboru vezme eventy a přidá je. Handler je ještě schopný nalézt jeden identifikátor, "self". Pokud je "self" nalezen a jeho hodnota je true, tak handler přidá, celý původní script, který byl volán. Toto umožnujě snadno vytvořit nekonečnou smyčku.

\subsubsection{Paměť programu}
Struktura paměti je takzvaný stack. Pro tento účel jsem vytvořil třídu, abych si mohl dodat manipulační metody, které bych mohl potřebovat. Samotný stack má struktu listu (toto jsem zvolil abych mohl použít indexované hledání, mazání, či přidávání hodnot), kdy hodnotou listu je vždy pole o dvou hodnotách. Na první pozici stojí vlastní hodnota a na druhé stojí id, nebo adresa, funkce či objektu, co tuhle hodnotu vytvořila. Pokud funkce chce pracovat s hodnotami jiných funkcí, tak je pužita první nalezená hodnota "shora" (Pokud bychom se dívali na hodnoty od indexu 0 až do X, tak by to byla první hodnota nalezená od nuly).
