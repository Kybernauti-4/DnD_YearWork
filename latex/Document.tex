...
Zobrazování je provedeno pomocí 8 LED na každé straně, které jsou ovládaný přes 8 bitový posuvný registr v časovém multiplexu pro ovládání dvou stran zároveň. Tento časový multiplex běží rychlostí 2kHz aby každá strana běžela na rychlosti 1kHz.

Schéma je v Přiloze 1. 
...
\subsection{Vlastní skripty}
Zde bych rád uvedl skripty, které jsem vytvořil a nějaký krátký popis, k čemu jsou určené.

\begin{description}
	\item[auto\_render] Tento skript ulehčuje a automatizuje práci s automatickým renderováním, které jsem popsal v sekci Okno Hry
	\item[comm] Toto nepovažuji úplně za skript, spíše za knihovnu pro všechny skripty, které vyžadují komunikaci s klíčem. V knihovně jsou vytvořené funkce, které umožňují různé čtení dat ze sériové linky a psaní dat na ní.
	\item[deviceHandler] Vyhledává všechny zařízení a jeho návratová hodnota je slovník zařízení, kde klíčem je hráčovo ID a hodnotou je sériová linka pro komunikaci.
	\item[eatVal] Odstaní hodnotu v argumentu ze stack paměti.
	\item[getCustomScripts] Hledá u zařízení skripty a ukládá je to adresáře player\_scripts
	\item[getPlayerList] Tento skript najde buď v klíčích nebo v adresáři players všechny hráče a pokusí se vytvořit jejich odpovídající objekty. Všechny podařené vrátí v listu.
	\item[Player] Toto je skript určený pro vytvoření objektu hráče. Při tvorbě bere jako argument název souboru s cestou kudy se tam dostat.
	\item[putVal] Vloží hodnotu poslanou po argumentu do stack paměti
	\item[storyHandler] Vyhledá všechny části příběhu, které mají být zprvu ukázány a vrátí list odpovídajících cest k nim.
	\item[Window] Popsáno v sekci okno hry.    
\end{description}
